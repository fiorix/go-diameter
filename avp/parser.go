...
func ReadAVP(r io.Reader, d *dict.Dict) (uint32, *AVP, error) {
	var (
		err error
		raw rfcHdr1
	)
	...
	var davp *dict.DictAVP
	if davp, err = d.AVP(avp.Code); err != nil {
	...
	if davp.Data.Type == "Grouped" {
		if eb, gavp, err := ReadAVP(r, d); err != nil {
			return 0, nil, err
		} else {
			if avp.Data == nil {
				avp.Data = Grouped{}
...
func (avp *AVP) String() string {
	/*
		...
		var name string
		if davp, err := dict.BaseDict.AVP(avp.Code); err != nil {
			name = "Unknown"
		} else {
			name = davp.Name
		}
