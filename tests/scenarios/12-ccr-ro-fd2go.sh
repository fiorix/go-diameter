#!/bin/bash
# 12-ccr-ro-fd2go: FD test_app client (tuned to CC command 272, App-Id 4)
# drives N CCR-like requests at the go CC server.
...
TEST_APP_MODE=1 TEST_APP_VENDOR=999999 TEST_APP_APP=4 \
...
wrapper=$(ct_wrapper_path "${go_log}" "${CT_BIN}/diam-cc-server"  -addr ":${GO_PORT}"  -origin-host "${GO_IDENTITY}" -origin-realm "${REALM}"  -app-id 4)
