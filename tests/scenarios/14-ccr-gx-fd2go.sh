#!/bin/bash
# 14-ccr-gx-fd2go: FD test_app client (Gx app, cmd 272) -> go cc-server.
...
TEST_APP_MODE=1 TEST_APP_VENDOR=10415 TEST_APP_APP=16777238 \
...
wrapper=$(ct_wrapper_path "${go_log}" "${CT_BIN}/diam-cc-server"  -addr ":${GO_PORT}"  -origin-host "${GO_IDENTITY}" -origin-realm "${REALM}"  -app-id 16777238  -extra-dicts "${CT_REPO}/diam/dict/testdata/gx_credit_control.xml")
sdme exec "${GO_CT}" -- /bin/sh "${wrapper}" &
go_pid=$!
trap 'kill_pids ${go_pid:-} ${fd_pid:-}; stop_processes_in "${GO_CT}" diam-cc-server' EXIT
wait_for_line "${go_log}" "cc server listening" 10
...
[[ "${ok}" -ge ${N} ]] || { log "expected >=${N} 2001, got ${ok}"; exit 1; }
